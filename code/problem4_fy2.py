...
import params as p
from evaluator import cover_intervals_cached_general, munition_pos_general, missile_pos_general 
...
def _cover_FY2(dt: float, v: float, dir_xy: np.ndarray,
               t_drop: float, t_burst: float,
               *, stage: int, precheck: bool):
    """
    兼容 evaluator 是否带 precheck 参数的两种签名。
    """
    try:
        return cover_intervals_cached_general(dt, v, dir_xy, t_drop, t_burst,
                                              p.FY2_0, p.M1_0, stage=stage, precheck=precheck)
    except TypeError:
        return cover_intervals_cached_general(dt, v, dir_xy, t_drop, t_burst,
                                              p.FY2_0, p.M1_0, stage=stage)


def _eval_total(x: np.ndarray, dt: float, stage: int, precheck: bool) -> float:
    v, dir_xy, t_drop, delay, t_burst = _decode(x)
    _, total = _cover_FY2(dt, v, dir_xy, t_drop, t_burst, stage=stage, precheck=precheck)
    return float(total)


def _smart_seeds_for_FY2_nominal() -> List[np.ndarray]:
    """
    生成一组智能初始种子，覆盖典型的投放参数组合。
    """
    NOM_TH_DEG = -45.0
    NOM_V      = 140.0
    NOM_TDROP  = 11.0
    ...
    dl_set = [NOM_DELAY + d for d in (-1.0, -0.6, 0.0, +0.6, +1.0)]
    v_set  = [NOM_V + d for d in (0.0, -6.0, +6.0, -12.0, +12.0)]
    ...
    for td in td_set:
        for dl in [NOM_DELAY-0.6, NOM_DELAY, NOM_DELAY+0.6]:
    ...
    for th in [th0 + math.radians(24), th0 - math.radians(24),
        ...
        seeds.append(_make(NOM_V, th, NOM_TDROP + 3.0, NOM_DELAY + 0.6))
    for th in [th0 + math.radians(12), th0 - math.radians(12)]:
        for v in [NOM_V-12, NOM_V-6, NOM_V+6, NOM_V+12]:
...
def plot_fy2_3d(FY0, v_drone, dir_xy, t_drop, delay, save_path, dt=0.05):
    """
    绘制三维场景图
    """
    ...
    ax.plot(FY_line[:,0],FY_line[:,1],FY_line[:,2],color="blue",   linewidth=2.2, label="UAV (FY2)")
    ...
    ax.set_title("Problem 4 (FY2) — Scene Overview")
    ax.legend(loc="best")
...
def run_problem4_FY2(cfg: Dict[str, Any]) -> None:
    ...
    seeds = _smart_seeds_for_FY2_nominal()
    ...
    def _log(g, best, x):
        if g % 4 == 0:
            print(f"[FY2] Gen {g:02d} best≈{best:.3f}s")
    ...
    print("\n=== FY2 — Single Bomb (Total-Length Objective) ===")
    ...
    intervals, total_length = _cover_FY2(
        dt=cfg["final_dt"], 
        v=v, 
        dir_xy=dir_xy,
        t_drop=t_drop, 
    ...
    ax.set_title("GA progress (Problem4-FY2)")
    ax.grid(True, linestyle='--', alpha=0.6)
    ax.legend()
    fig.tight_layout()
    fig.savefig("../figures/fy2_ga_progress.png", dpi=180, bbox_inches="tight")
    plt.show()
    ...
    plot_fy2_3d(
        ...
        t_drop=t_drop,
        delay=delay,
        save_path="../figures/problem4_fy2_3d.png",
        dt=0.05
    )
